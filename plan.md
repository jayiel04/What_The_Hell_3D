...
- [~] Conectar malla y animaciones de personajes importadas; la malla del caballero (`Knight_Male.gltf`) y las 5 de enemigos (Goblin_M/F, Zombie_M/F, Witch) se cargan en ejecución desde `Assets/StreamingAssets/Characters/` vía glTFast y se animan con sus clips embebidos (Idle/Walk/Run/Jump/SwordSlash/Roll/RecieveHit/Death) según estado de combate; pendiente: equipar arma en hueso de mano, retargetear a humanoide para mezclar fuentes y VFX finales. Los sonidos de combate (espada, parry/bell, daño) ya están cableados vía `PlayerController.combatAudioSource` y los hooks de VFX existen (`attackVfx`/`parryVfx`).
...
- [x] Crear objetos serializados de goblin, zombie y bruja —candidatos a prefabs— desde Unity Editor con `EnemyController` y `HealthComponent`, con malla glTF visible cargada en ejecución desde `StreamingAssets` (5 modelos: Goblin_M/F, Zombie_M/F, Witch) y animación por estado.
